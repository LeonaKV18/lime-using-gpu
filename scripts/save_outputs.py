...
Loads X.bin, preds.bin, weights.bin and saves them as a single NumPy .npz
...
def main():
    ...
    np.savez(args.out, X=X, preds=preds, weights=weights)
    ...
    print(f"\nTo load later:  data = np.load('{args.out}')  then data['X'], data['preds'], data['weights']")
